            ...
            for row_off in range(-1, 2):
                for column_off in range(-1, 2):
                    if ((row_off == 0 or column_off == 0) and row_off != column_off
                        ...
                        if not game.grid[row + row_off][column + column_off].is_visible and not \
                        game.grid[row + row_off][column + column_off].has_bomb:
                            ...
                            if game.grid[row + row_off][column + column_off].bomb_count == 0:
                                game.grid[row + row_off][column + column_off].open_neighbours(game.squares_y,
                                                                                              game.squares_x)
class Menu():
    def __init__(self):
    ...
    def draw(self, obj):
        ...
        if obj.game_lost:
            self.label_game_end.show(screen, "Вы проиграли!")
        elif obj.game_won:
            self.label_game_end.show(screen, "Вы победили!")
    class Label:
        def __init__(self, x, y):
            self.x = x
            self.y = y
    ...
    class Button:
        ...
        def click_handle(self):
            ...
            if pos[0] > self.x and pos[1] > self.y and pos[0] < (self.x + self.width) and pos[1] < (
                    self.y + self.height):
                return True
            else:
                return False
...
screen = pygame.display.set_mode(size, pygame.RESIZABLE)
pygame.display.set_caption("Minesweeper for ¡nFactorial!")
